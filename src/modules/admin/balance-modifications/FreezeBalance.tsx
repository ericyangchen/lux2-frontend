...
export function FreezeBalance() {
  ...
  const handleFreezeBalance = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await ApiFreezeBalance({
        organizationId,
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: `已為組織 ${organizationId} 凍結 ${
            amount ? Calculator.toFixedForDisplay(amount) : "0.000"
          } ${paymentMethod}`,
          variant: "success",
        });
  ...
  return (
    <>
      <div className="sm:p-4 sm:border rounded-md w-full lg:h-[calc(100vh-152px)] lg:overflow-y-scroll">
        <Label className="whitespace-nowrap font-bold text-md">凍結餘額</Label>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>凍結成功</DialogTitle>
            <DialogDescription>餘額凍結操作已成功完成</DialogDescription>
          </DialogHeader>
          <DialogFooter>
